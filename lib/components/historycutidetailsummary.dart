...
class HistoryCutiSum extends StatelessWidget {
  final String nik;
  final String ket;
  const HistoryCutiSum({Key?key,required this.nik,required this.ket}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    debugShowCheckedModeBanner: false,
      home: HistoryCutiDetailSummary(nik: nik, ket: ket)
    );
  }
}


class HistoryCutiDetailSummary extends StatefulWidget {
  final String nik;
  final String ket;

  const HistoryCutiDetailSummary({Key?key,required this.nik,required this.ket }):super(key: key)  ;
  ...
  @override
  State<HistoryCutiDetailSummary> createState() => _HistoryCutiDetailSummaryState( nik:nik,ket:ket);
}

class _HistoryCutiDetailSummaryState extends State<HistoryCutiDetailSummary> {
 final String nik;
  final String ket;
  _HistoryCutiDetailSummaryState({Key?key,required this.nik,required this.ket })  ;
 ...
 Future <void> getRefreshdata() async{
  Provider.of<MapDatas>(context,listen:false ).getHistoryCutiSum(nik,ket);
 }


  Future <void> getRefreshdata2() async{
  Provider.of<MapDatas>(context,listen:false ).getHistoryCuti(nik,ket);
 } 
                    ...
                    return ListTile(
                        ...
                        GestureDetector(
                         child: 
                          ...
                          child: Text
                        (prov.datamap[i].tglrec!+"("+prov.datamap[i].absen!+")",style: TextStyle(fontSize: 10,fontWeight: FontWeight.bold,color: Colors.white)),
                        ),
  ...
  onPopInvoked : (didPop){
   // logic
    ...
    child: 
     ...
     appBar: AppBar(title: Text("History "+ket.toUpperCase(),style: TextStyle(fontWeight: FontWeight.bold,fontSize: 20,color: Colors.white),),
            ...
            children: [
             ...
             SingleChildScrollView(
              child: Container(
                ...
                child: 
                FutureBuilder(future: Provider.of<MapDatas>(context,listen: false).getHistoryCutiSum(nik,ket)
                , builder: (context, snapshot) {
                   if (snapshot.connectionState==ConnectionState.waiting)
                   {
                    ...
                    return ListView.builder(
                      itemCount: prov.datamapsum.length,
                      itemBuilder: (context, i) {
                        return Card(
...
SizedBox(height: 5,) ,                                            
//Expanded(child: Text('Jumlah Cuti',style: TextStyle(backgroundColor: Colors.black,color: Colors.white,fontWeight: FontWeight.bold),textAlign: TextAlign.center)),                              
//SizedBox(height: 5,),
                              Container(
                                ...
                                Row(children: [
   SizedBox(width: 100,child:  Text('Jumlah Cuti : ',style: TextStyle(color: Colors.white),),),
    Text(prov.datamapsum[i].absen!.trimLeft()+"    (Click For Detail)            ",style: TextStyle(color: Colors.white),),
                                ],),
...
SizedBox(height: 5,),
  Container(
    ...
    child:  
    Row(children: [
    SizedBox(width: 100,child:  Text('Sisa Cuti : ',style: TextStyle(color: Colors.white),),),
    Text(prov.datamapsum[i].pict_att!.trimLeft(),style: TextStyle(color: Colors.white),),
                                ],),
